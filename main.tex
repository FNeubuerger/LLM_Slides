...
\begin{frame}{Was ist eine Ontologie? -- Grundlagen}
  \begin{itemize}
    ...
    \item \textbf{Vier Kernkomponenten:}
      \begin{itemize}
        \item \textbf{Klassen (Concepts):} Abstrakte Kategorien, z.\,B. \texttt{Krankheit}, \texttt{Medikament}.
        \item \textbf{Properties (Relationen):} Typisierte Verbindungen, z.\,B. \texttt{behandelt}, \texttt{istTeilVon}.
        \item \textbf{Instanzen (Individuals):} Konkrete Objekte, z.\,B. \texttt{Metformin} $\in$ \texttt{Medikament}.
        \item \textbf{Axiome (Restrictions):} Logische Regeln, z.\,B.\\
          \texttt{DiabetesTyp2} $\sqsubseteq$ \texttt{Stoffwechselerkrankung}
      \end{itemize}
    \item \textbf{Technologien:} OWL~2 (Web Ontology Language), RDF/RDFS, SKOS.
    \item \textbf{Abfragesprache:} SPARQL -- SQL für Graphen und Ontologien.\\
      Bsp.: \texttt{SELECT ?t WHERE \{ :DiabetesTyp2 :behandeltMit ?t \}}
  \end{itemize}
\end{frame}
...
\begin{frame}{Ontologien: Competency Questions -- Anforderungen formalisieren}
  \begin{itemize}
    ...
    \item \textbf{Zweck:}
      ...
      \end{itemize}
    \item \textbf{Beispiel-CQs (medizinische Ontologie):}
      \begin{itemize}
        \item ``Welche Medikamente werden zur Behandlung von Diabetes~Typ~2 eingesetzt?''
        \item ``Welche Kontraindikationen existieren für Metformin?''
        \item ``Welche Erkrankungen sind Subklassen von Stoffwechselerkrankungen?''
      \end{itemize}
...
\begin{frame}{Ontologien: Menschliche Kuration und etablierte Beispiele}
  \begin{itemize}
    ...
    \item \textbf{Etablierte Ontologien:}
      \begin{itemize}
        \item \textbf{Medizin:} SNOMED CT ($>$350\,000 Konzepte), ICD-10/11 (WHO), LOINC, Gene Ontology (GO)
        \item \textbf{Allgemein:} Schema.org (Web), Wikidata, DBpedia, FOAF
        \item \textbf{Industrie:} Asset Administration Shell (AAS/RAMI~4.0), ISO~15926
        \item \textbf{Wissenschaft:} ChEBI (Chemikalien), OBO Foundry (biologische Domänen)
      \end{itemize}
...
\begin{frame}{Vom NER-Ergebnis zur Ontologie: Entity Linking}
  \begin{itemize}
    ...
    \vspace{0.2cm}
    \item \textbf{Beispiel:}\\
      Text: ``Der Patient hat \textbf{Diabetes}.''\\
      NER: \texttt{Diabetes} $\rightarrow$ DISEASE\\
      EL + Kontext: \texttt{Diabetes} $\rightarrow$ \texttt{SNOMED:73211009} (Diabetes mellitus, nicht spezifiziert)
    \item \textbf{Werkzeuge:} BLINK (Meta), mGENRE (mehrsprachig), scispaCy (biomedizinisch), ReFinED.
  \end{itemize}
\end{frame}
